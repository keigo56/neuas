...
<nav class="bg-white shadow" x-data="{ open : false }">
    ...
    <div
        ...
        <div class="px-2 pt-2 pb-3 space-y-1">
            @foreach($routes as $key => $route)
                <a href="{{ $route['path'] }}" class="@if($selected === $key) bg-gray-900 text-white @else text-gray-300 hover:bg-gray-700 hover:text-white @endif block px-3 py-2 rounded-md text-base font-medium"  @if($key === $selected) aria-current="page" @endif>{{ $route['name'] }}</a>
            @endforeach
        </div>
    </div>
</nav>
